...
class Models_punto_venta extends CI_Model{
    ...
    public function actualiza_pago_deuda($total_deuda,$nombre)
    {
        ...
        $pago_deu=$this->db->query("insert into total_venta(n_venta,total_venta,fecha) values('0','".$total_deuda."','".$fecha_d_m_y."')");
    }
    ...
    {
        $t_venta=$this->db->query("select sum(valor) as total from ventas where estado='0' ");
        if($t_venta!=false)
        {
            return $t_venta->result_array();
        }
    }
    public function f_venta()
    {
        $fi_venta=$this->db->query("select max(n_venta)+1 as n_venta from ventas");
        if($fi_venta!=false)
        {
            return $fi_venta->result_array();
        }
    }
    public function act_n_venta($n_venta,$total_venta)
    {
        $this->db->query("update ventas set n_venta='".$n_venta."',estado='1'  where estado='0' ");
        $fecha = new DateTime();
            $fecha_d_m_y = $fecha->format('Y-m-d h:s');
            $fecha_d_m_y;
        ...
        $this->db->insert('total_venta',$t_venta);
    }
    public function ven_pendiente()
    {
        $vent_pendiente=$this->db->query("select * from ventas where estado='0'");
        if($vent_pendiente!=false)
        {
            return $vent_pendiente->result_array();
        }
    }
